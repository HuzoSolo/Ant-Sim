...
class Setup:
    ...
    def create_objects(self):
        ...
        rand_locations = [(random.randint(0, WIDTH-100), random.randint(0, HEIGHT-100)) for _ in range(30)]
        # yiyecek konum ve sayısının belirlenmesi
        
        for location in rand_locations:
            temp_foods = pygame.sprite.Group()
            temp_foods = pygame.sprite.Group(
            [Food(random.randint(location[0], location[0]+50),random.randint(location[1], location[1]+50), "food") for i in range(20)])
